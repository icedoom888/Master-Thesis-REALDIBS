...
from .sullens_morphed_dataset import sullens_morphed_dataset
from .trinity_crop_morphed_dataset import trinity_crop_morphed_dataset
from .sullens_morphed_animation_dataset import sullens_morphed_animation_dataset
...
class DataManager:
    ...
    resize( input_image=None, real_image=None, mask=None )
        resizes input, morphed, target and mask images to a given factor. Necessary for the artgan network
    ...
    store_generated_img_str( prediction=None, name=None )
        stores the given 'prediction' image into the 'output_dir' directory using 'name' as the given file name
    ...
    def preprocess_train(self, datapoint):
        ...
        ----------
        datapoint: dict
            a datapoint of the dataset, keys: in_img, morph_img, tar_img, mask
        ...
        morph_image = tf.cast(datapoint['morph_img'], tf.float32)
        ...
        if self.flags.method == 'artgan':
            ...
            input_image, morph_image, real_image, mask = self.resize(input_image, morph_image, real_image, mask)

        else:
            pass

        # pre-process: random crop and jitter plus normalisation
        input_image, morph_image, real_image, mask = self.random_jitter(input_image, morph_image, real_image, mask)
        input_image, morph_image, real_image, mask = self.normalize(input_image, morph_image, real_image, mask)

        datapoint['in_img'] = input_image
        datapoint['morph_img'] = morph_image
    ...
    def preprocess_test(self, datapoint):
        ...
        try:
            ...
            real_image = tf.zeros((self.flags.height, self.flags.width, 3))

        try:
            mask = tf.cast(datapoint['mask'], tf.float32)
        except KeyError:
            mask = tf.zeros((self.flags.height, self.flags.width, 3))

        # the artgan network requires cropping during testing as well
        if self.flags.method == 'artgan':
            input_image, morph_image, real_image, mask = self.resize(input_image, morph_image, real_image, mask)
            input_image, morph_image, real_image, mask = self.pad_test(input_image, morph_image, real_image, mask)
            input_image, morph_image, real_image, mask = self.random_jitter(input_image, morph_image, real_image, mask)
        else:
            input_image, morph_image, real_image, mask = self.pad_test(input_image, morph_image, real_image, mask)
    ...
    @tf.function
    def resize(self, input_image, morph_image, real_image, mask):
        """
        resizes input, morphed, target and mask images to a given factor. Necessary for the artgan network
        ...
        morph_image: tf.image
            morphed image from meshoptim
        ...
        if self.flags.resize:
            ...
            morph_image = tf.image.resize(morph_image, (height, width),
                                          method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        ...
        return input_image, morph_image, real_image, mask

    def random_crop(self, input_image, morph_image, real_image, mask):
        """
        ...
        Returns
        -------
        cropped patches of each received image

        """

        stacked_image = tf.stack([input_image, morph_image, real_image, mask], axis=0)  # stack the 4 images
        # Crop the stacked image
        cropped_image = tf.image.random_crop(stacked_image, size=[4, self.flags.patch_size, self.flags.patch_size, 3])

        return cropped_image[0], cropped_image[1], cropped_image[2], cropped_image[3]

    def normalize(self, input_image, morph_image, real_image, mask):
        """
        normalizes the given images
        ...
        morph_image = (morph_image / 127.5) - 1
    ...
    @tf.function
    def random_jitter(self, input_image, morph_image, real_image, mask):
        """
        combines the pre-processing operations of random cropping and random horizontal flipping.
        ...
        """
        input_image, morph_image, real_image, mask = self.random_crop(input_image, morph_image, real_image, mask)
        if tf.random.uniform(()) > 0.5:  # random mirroring
            input_image = tf.image.flip_left_right(input_image)
            morph_image = tf.image.flip_left_right(morph_image)
            real_image = tf.image.flip_left_right(real_image)
            mask = tf.image.flip_left_right(mask)

        return input_image, morph_image, real_image, mask

    def pad_test(self, input_image, morph_image, real_image, mask):
        """
        padding function to be used for pre-processing at test time
        ...
        stacked_image = tf.stack([input_image, morph_image, real_image, mask], axis=0)
        padded_image = tf.pad(stacked_image, self.paddings, "CONSTANT")

        return padded_image[0], padded_image[1], padded_image[2], padded_image[3]
    ...
    def store_generated_img(self, prediction, input, tar, epoch):
        ...
        if epoch == 0:  # store target and input images if epoch is 0
            ...
            p = self.remove_pad(p)  # remove pad
            img = p[0]  # get stacked image

            # input image is in the first 3 channels
            in_img = img[:, :, :3]
            in_img = tf.image.encode_png(in_img)  # encode png
            tf.io.write_file(self.output_dir + 'in' + '.png', in_img)  # writes the image to file

            # morphed image is in the last 3 channels
            morph_img = img[:, :, 3:]
            morph_img = tf.image.encode_png(morph_img)  # encode png
            tf.io.write_file(self.output_dir + 'morph' + '.png', morph_img)  # writes the image to file
    ...
    def store_generated_img_str(self, prediction, name):
        ...
        p = self.remove_pad(p)  # remove pad
        img = tf.image.encode_png(p[0])  # encode png
        tf.io.write_file(self.output_dir + name, img)  # write the image to file
